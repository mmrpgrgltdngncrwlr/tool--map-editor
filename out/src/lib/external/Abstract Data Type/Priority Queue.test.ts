...
function buildComplexTests(Queue: new (isOrdered: (a: { v: number }, b: { v: number }) => boolean) => IPriorityQueue<{ v: number }>, isOrdered: (a: { v: number }, b: { v: number }) => boolean, count: number, expectedMapper: (expected: { v: number }[]) => { v: number }[] = (expected) => expected) {
