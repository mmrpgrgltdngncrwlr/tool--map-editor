  ...
  static CreateServer(port) {
    const server = Bun.serve({
      ...
      routes: {
        ...
        "/authenticated/*": (req) => {
          return HOOK_RES.Async_CatchInternalServerError(() => HOOK_REQ.Async_AuthenticateAccessToken(req, () => SERVER.Async_GetResource(req), () => Response.redirect("/pairing.html")));
        },
        "/*": (req, server2) => {
    ...
    try {
      ...
      if (error !== null && typeof error === "object") {
        if ("code" in error && error.code === "EADDRINUSE")
          code = "EADDRINUSE";
        if ("message" in error && error.message === `Failed to start server. Is port ${port} in use?`)
          code = "EADDRINUSE";
      }
      if (code === "EADDRINUSE") {
...
class TOKEN_RECORD {
  ...
  static PARSE(record) {
    return [
      record[TOKEN_ID.TOKEN_HASH],
      record[TOKEN_ID.CREATED_AT],
      record[TOKEN_ID.EXPIRES_AT]
    ];
  }
}
